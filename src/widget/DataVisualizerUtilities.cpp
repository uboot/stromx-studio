...
namespace
{
        ...
        const VisualizationState::Properties & visualizationProperties)
    {
        ...
        if(data.isVariant(DataVariant::INT_8_MATRIX))
            return createHistogramItemsTemplate<int8_t>(data, visualizationProperties);
        else if(data.isVariant(DataVariant::UINT_8_MATRIX))
            return createHistogramItemsTemplate<uint8_t>(data, visualizationProperties);
        else if(data.isVariant(DataVariant::INT_16_MATRIX))
            return createHistogramItemsTemplate<int16_t>(data, visualizationProperties);
        else if(data.isVariant(DataVariant::UINT_16_MATRIX))
            return createHistogramItemsTemplate<uint16_t>(data, visualizationProperties);
        else if(data.isVariant(DataVariant::INT_32_MATRIX))
            return createHistogramItemsTemplate<int32_t>(data, visualizationProperties);
        else if(data.isVariant(DataVariant::UINT_32_MATRIX))
            return createHistogramItemsTemplate<uint32_t>(data, visualizationProperties);
        else if(data.isVariant(DataVariant::FLOAT_32_MATRIX))
            return createHistogramItemsTemplate<float>(data, visualizationProperties);
        else if(data.isVariant(DataVariant::FLOAT_64_MATRIX))
            return createHistogramItemsTemplate<double>(data, visualizationProperties);
        else
            return QList<QGraphicsItem*>();
    }
    ...
    const VisualizationState::Properties & visualizationProperties)
{
    ...
    switch(visualizationVariant.toInt())
    {
        ...
        case AbstractDataVisualizer::HISTOGRAM:
            return createHistogram(data, visualizationProperties);
